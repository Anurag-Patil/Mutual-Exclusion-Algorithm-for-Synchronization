...
bool test_and_set();
void entry_sec_tas(int i,thread::id id, int k);
void exit_sec_tas(int i,thread::id id);
void tas_testcs(int n, int k, int j, double tt1, double tt2);
void tas(int n, int k);
...
int main()
{
	...
	fout.open("tas-output.txt");

	tas(n,k);
	
	fout.close();

	fout.open("tas-stat.txt");
...
void tas(int n, int k)
{
	...
	for(int i = 0; i < n; i++)
	{
		...
		th.push_back(thread(tas_testcs,n,k,i,tt1,tt2));
	}
	for(thread &t : th)
	{
		t.join();
