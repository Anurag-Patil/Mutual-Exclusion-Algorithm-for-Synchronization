...
void entry_sec_cas(int i,thread::id id, int k);
void exit_sec_cas(int i,thread::id id);
void cas_testcs(int n, int k, int j, double tt1, double tt2);
void cas(int n, int k);
...
int main()
{
	...
	fout.open("cas-output.txt");

	cas(n,k);

	fout.close();

	fout.open("cas-stat.txt");
...
void cas(int n, int k)
{
	...
	for(int i = 0; i < n; i++)
	{
		...
		th.push_back(thread(cas_testcs,n,k,i,tt1,tt2));
	}
	for(thread &t : th)
	{
		t.join();
